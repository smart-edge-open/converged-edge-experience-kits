...
def make_parser():
    ...
    parser.add_argument(
        ...
        default="https://github.com/otcshare/openness-experience-kits",
    ...
    parser.add_argument(
        "hosts",
        help="Comma separated IP addresses of edge hosts to provision. First one is always the controller."
             "If there is only one IP provided then single node cluster is provisioned")
    parser.add_argument(
...
def clone_repository(git_repo, git_token, path):
    ...
    try:
        ret = subprocess.run(
            ["/usr/bin/git", "clone", urlunparse(git_url), path])
        _LOG.info("git finished: {}".format(ret.returncode))
    except subprocess.CalledProcessError as e:
        _LOG.fatal("\"{}\" failed[{}]: {}".format(
...
def main(options):
    ...
    try:
        _LOG.debug("Running '{}'".format(" ".join(command)))
        ret = subprocess.run(command)
        if ret.returncode:
            _LOG.error("Installation failed({})".format(ret.returncode))
